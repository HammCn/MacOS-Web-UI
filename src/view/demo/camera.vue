<!--
 * @FilePath: /MacOS/src/view/demo/camera.vue
 * @Author: admin@airmole.cn
 * @Date: 2021年08月14日13:56:13
 * @LastEditTime: 2021-08-14 13:56:25
 * @LastEditors: admin@airmole.cn
 * @Description: Photo Booth相机
-->
        ...
        <div class="preview">
            ...
            <div class="shooted-list" :style="nowPreview ? `top: -${height}px` : 'top: -60px'" v-if="shootedList.length > 0">
                <div style="display: inline-block;" v-for="(photo, index) in shootedList" v-bind:key="photo">
                    <div class="shooted-del"><i @click="deletePhoto(index)" class="el-icon-error"></i></div>
                    <img @click="preview(photo)" class="photo" :src="'data:image/png;base64,' + photo"/>
                </div>
            </div>
        </div>
        <div class="toolbar" v-if="canUse">
            <el-radio-group :style="nowPreview ? 'visibility:hidden;' : ''" v-model="mode" size="mini" text-color="#fff" fill="#A0A0A0">
